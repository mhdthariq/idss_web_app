...
import { CONT_FEATURES, CAT_FEATURES } from "./features";
...
export async function loadMLPModel(
  ...
  weights = {
    embeddings,
    input_proj: {
      weight: raw["input_proj.weight"],
      bias: raw["input_proj.bias"],
    },
    residual_blocks: [0, 1, 2, 3].map((i) => ({
      weight: raw[`residual_blocks.${i}.linear.weight`],
      bias: raw[`residual_blocks.${i}.linear.bias`],
    })),
    output: {
      weight: raw["output.weight"],
      bias: raw["output.bias"],
    },
  };
...
export async function predictMLP(
  ...
  const contValues = CONT_FEATURES.map((f, i) => {
  ...
  let h = linearForward(input, weights.input_proj.weight, weights.input_proj.bias);
  h = applyGELU(h);
